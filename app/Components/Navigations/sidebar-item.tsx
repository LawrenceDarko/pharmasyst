...
return (
    ...
    <div
        ...
        className={cn('flex items-center cursor-pointer mx-0 gap-x-2 text-white text-sm font-[500] pl-6 transition-all hover:text-white hover:bg-slate-300/20',
    ...
    {submenu && (
        ...
        {submenu.map(({ href, name, SubIcon }, index) => (
            <ListItem
            key={index}
            className={`flex py-4 w-full items-center cursor-pointer text-gray-300 text-sm font-[500] transition-all hover:text-white hover:bg-slate-300/20`}
            >
...
// const SidebarItem: React.FC<SidebarItemProps> = ({ icon: Icon, label, href, submenu }) => {
//     const { isSubmenuOpened, setIsSubmenuOpened } = useGeneralContext()
